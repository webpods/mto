...
# XMLRPC
if (($globlogs{CUSTOM9_LOG}{$lgfile}) and ($line =~ /(\S+).*] "\w*(?:GET|POST) \/xmlrpc\.php.*" /)) {
    return ("Failed XMLRPC Attempts",$1,"XMLRPC","10","80,443","1800");
    }

#WP-LOGINS
if (($globlogs{CUSTOM9_LOG}{$lgfile}) and ($line =~ /(\S+).*] "\w*(?:POST) \/wp-login\.php.*" /)) {
    return ("Failed WP-LOGIN Attempts",$1,"WPLOGIN","15","80,443","1800");
    }
